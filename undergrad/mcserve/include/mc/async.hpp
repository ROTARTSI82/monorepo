...
template <typename T> struct pool_future;

struct task_rescheduler;
struct unit {};

template <typename P> struct defer {
  P fun;
  ~defer() { fun(); }
};
...
template <typename T> struct pool_promise : pool_promise_generic {
  std::optional<T> ret;
...
template <> struct pool_promise<void> : pool_promise_generic {
  std::suspend_always initial_suspend() { return {}; }
  std::suspend_always final_suspend() noexcept { return {}; }
  void return_void() {}
  void unhandled_exception() {}
...
// internal base type to allow handling the special behavior of
// pool_future<void> since it's a quirky boy
template <typename T> struct _pool_future {
...
template <typename T> struct pool_future : public _pool_future<T> {
  inline std::optional<T> &value() { return this->handle.promise().ret; }
};

template <> struct pool_future<void> : _pool_future<void> {};
...
struct pool_task {
  ...
  inline operator bool() const { return handle && promise; }
};
...
public:
  ...
  template <typename P> void await_suspend(std::coroutine_handle<P> h) {
...
public:
  ...
  // correctly handle returning from a task or destroying it
  // if it's not owned by any caller
  inline void task_return(const pool_task &task, bool cancel) {
  ...
  template <typename T> inline void queue(pool_future<T> &&fut) {
...
struct io_awaiter : public task_rescheduler {
  ...
  io_awaiter(ssize_t bytes) : bytes(bytes), suspended(false) {};
...
template <typename T> struct pool_awaiter : task_rescheduler {
  ...
  T &&await_resume() { return std::move(*(callee.promise().ret)); }
};
...
public:
  ...
  template <typename P>
  pool_future<bool> set_interval(unsigned long sec, unsigned long ns, P pred,
                                 std::stop_token tok = never_stop.get_token()) {
    itimerspec utmr = {};
