...
void main() {
    dvec2 orig = sample_bl_io + mod(tex_co, sample_extent_io);
    dvec2 current = dvec2(0, 0);
    uint iter = 0;
    ...
    while (length(current) < MAX_MAGN && iter < MAX_ITER) {
        ...
        current = dvec2(current.x * current.x - current.y * current.y, 2 * current.x * current.y) + orig;
        iter++;
    }
