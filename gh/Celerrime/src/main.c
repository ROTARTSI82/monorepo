...
int main() {
    ...
    app.view_mat = *(smat4 *) sm4_identity; // mumble mumble aliasing
    ...
    app.renderer.local_array[0].num_tiles.x = 25;
    ...
    app.renderer.local_array[0].transform = sm2_transform(&tmp_scale, 0);

    app.renderer.local_array[0].translate.x = 0;
    app.renderer.local_array[0].translate.y = -2.5;
    app.renderer.local_array[0].translate.z = -42;
